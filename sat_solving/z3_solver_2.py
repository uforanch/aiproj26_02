...
"""
start with random plane try changing signs
"""
...
for i_k in range(k):
    ...
    for i_d in range(d):
        ...
        a_coeff[-1].append(random.random())
b=[]
for i_k in range(k):
...
for i_l in range(l):
    int_mat.append([])
    for i_k in range(k):
        edge_const = z3.Or(z3.And(sum([a[i_k][i_d] * a_coeff[i_k][i_d] * hp1[i_d, i_l] for i_d in range(d)])- b[i_k]>0,
...
if debug:
    ...
    for i_l in range(l):
        for i_k in range(k):
            ...
            p1=sum([h[i_k,i_d] * hp1[i_d, i_l] for i_d in range(d)])- h[i_k,d]
            p2=sum([h[i_k,i_d] * hp2[i_d, i_l] for i_d in range(d)])- h[i_k,d]
            c1 = sum([a[i_k][i_d] * hp1[i_d, i_l] for i_d in range(d)])- b[i_k]>=0
            c2 = sum([a[i_k][i_d] * hp2[i_d, i_l] for i_d in range(d)]) - b[i_k] <= 0
            c3 = sum([a[i_k][i_d] * hp1[i_d, i_l] for i_d in range(d)]) - b[i_k] <= 0
            c4 = sum([a[i_k][i_d] * hp2[i_d, i_l] for i_d in range(d)]) - b[i_k] >= 0
